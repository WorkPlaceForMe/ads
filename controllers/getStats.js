        ...
        if (err) {
          ...
          getClicksAndViews(async function (err, rows) {
            if (err) {
              ...
              res
                .status(200)
                .json({ success: true, table: table, rewards: rewards })
            }
          })
        }
      })
          ...
          for (let i = 0; i < rows.length; i++) {
            ...
            for (const resCsv of results) {
              if (parseInt(Object.values(resCsv)[0]) == rows[i].idItem) {
                ...
                ads.push({
                  img: resCsv['Image URL'],
                  title: resCsv['Merchant Product Name'],
                  affiliate: resCsv['Product URL Web (encoded)'],
                  views: view,
                  clicks: click,
                  ctr: ctr,
                })
...
function getAdsList(img, site, callback) {
  return db.query(
    `SELECT imgName, idGeneration,idItem FROM ${conf.get(
      'database',
    )}.adsPages where site = '${site}' and imgName='${img}' order by idGeneration desc;`,
    callback,
  )
}
...
function getAdsClicksAndViews(img, site, callback) {
  return db.query(
    `SELECT idItem,img, COUNT( CASE WHEN type = '2' THEN 1 END ) AS clicks, COUNT( CASE WHEN type = '1' THEN 1 END ) AS views FROM ${conf.get(
      'database',
    )}.impressions where url = '${site}' and img= '${img}' group by idItem;`,
    callback,
  )
}
