    ...
    return new Promise((resolve, reject) => {    
      data.pipe(parseCsv({ delimiter: ',', from_line: 2, headers: true }))
          ...
          try {          
            for (const element of objetos['Products']) {
              if (csvrow[15] && 
                ...
                const product = createProducts(csvrow, element, id, 'Tops Online')
            ...
            if (csvrow[13] && csvrow[13].toLowerCase().includes('beauty')){
              const product = createProducts(csvrow, 'makeup', id, 'Tops Online')
              promises.push(product)
            }
          } catch(err) {
        ...
        .on('end', async () => {
          ...
          console.log(`Completed setup for site ${id} for provider Tops Online in ${responseTimeMs}ms`)      
