...
const clothing_Filler = (
  ...
  if (gender == 'Male') {
    if ((obj.class == 'person' || obj.class == 'upper' && obj.confidence > 0.5)) {
      if (obj.deep_fashion_tf.sleeve_length[0].label == 'ExtraLongSleeves' ||
        ...
        const results = objetos.filter(
          (obj2) =>
            obj2.Gender == gender && obj2.Sub_Category_Name.toLowerCase().includes('jacket') && !obj2.Main_Category_Name.toLowerCase().includes('women')
        )
        ...
        if(count >= 0){
          // console.log(results)
            resultsAffiliate_Temp.push({
              vista: obj,
              affiliate: results[int],
              add: {
                id: parseInt(results[int]['Merchant_Product_ID']),
                site: site,
                date: dateFormat(new Date(), 'yyyy-mm-dd HH:MM:ss'),
                url: url,
                uid: uid,
                product_site_url: results[int]['Product_URL_Web_encoded'],
                product_image_url: results[int]['Image_URL'],
                product_main_category_name: results[int]['Main_Category_Name'],
                vista_keywords: 'ExtraLongSleeves Upper,LongSleeves Upper',
                provider_name: results[int]['Provider_Name']
              },
              serv: serv,
        ...
        const results = objetos.filter(
          (obj2) => obj2.Gender == gender && obj2.Sub_Category_Name.toLowerCase().includes('tops') 
          && !obj2.Main_Category_Name.toLowerCase().includes('women')
        )
        const count = results.length - 1
        let int = Math.floor(Math.random() * count)
    ...
    if (obj.class == 'person' && obj.confidence > 0.5) {
      if (
  ...
  if (gender == 'Female') {
    if (obj.class == 'person' || obj.class == 'upper' && obj.confidence > 0.5) {
      if (obj.deep_fashion_tf.sleeve_length[0].label == 'ExtraLongSleeves' ||
        ...
        const prendras = objetos.filter((obj2) => obj2.Sub_Category_Name.toLowerCase().includes('jackets')&& obj2.Main_Category_Name.toLowerCase().includes('women'))        
        ...
        if(count >= 0){
          ...
          resultsAffiliate_Temp.push({
            ...
            add: {
              ...
              vista_keywords: 'ExtraLongSleeves Upper,LongSleeves Upper',
              provider_name: prendras[int]['Provider_Name']
            },
            serv: serv,
            size: { w: img_width, h: img_height },
            mobile: mobile,
...
const object_Filler = (
  ...
  const results = objetos.filter(
    (obj2) => 
      ...
      obj2.Type == 'products' && obj.confidence > 0.5
  )
...
const sport_makeup_Filler = (
  ...
  if (bool) {
    ...
    if (obj.class && obj.class.toLowerCase().includes('sport')) {
      ...
      console.log("Results sport product in sport") 
      const count = results.length - 1
      let int = Math.floor(Math.random() * count)
  ...
  if (obj.label && (obj.label.toLowerCase().includes('lipstick') || obj.label.toLowerCase().includes('hair') || obj.label.toLowerCase().includes('face')
      ...
      console.log("Results from beauty/make up 2 - Lipstick,Hair,Perfume,Paintbrush") 
      const count = results.length - 1
      let int = Math.floor(Math.random() * count)
